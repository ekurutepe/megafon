    ...
    rfg.ajax.makeFlickrCalls = function (infoApiCall, singleSet) {
        $.getJSON(infoApiCall, function (data) {
            ...
            var apiCall = "http://api.flickr.com/services/rest/?format=json&method=flickr.photosets.getPhotos&photoset_id=" +
                singleSet.setId + "&per_page=18&page=1&api_key=" + singleSet.apiKey + "&jsoncallback=?&extras=description, license, date_upload, date_taken, owner_name, icon_server, original_format, last_update, geo, tags, machine_tags, o_dims, views, media, path_alias, url_sq, url_t, url_s, url_q, url_m, url_n, url_z, url_c, url_l, url_o";

            $.getJSON(apiCall, function (data) {
                var imageCount = 0; //total images data.photoset.photo.length
                $.each(data.photoset.photo, function (i, photo) {
                    var type = 'url_' + rfg.options.flickrOptions.imageSize;
                    var img_src = photo[type];
